...
2. 检查宽带是否有开通 IPv6 的条件，如果还不支持 IPv6 那么无法开通，只能换家运营商或者等运营商方面升级。有的时候宽带已经支持 IPv6，但是路由器并没有开启。路由器开启 IPv6，可以百度搜索`路由器品牌 + 怎么开 IPv6`。这里列举几种品牌路由器的 IPv6 的开启方法
    - 华为路由器：登录后台后，点击顶部更多功能，侧边栏找到网络设置点击展开，选择 IPv6，打开开关，下面所有选项全选择自动配置，点击保存。
      切换到侧边栏里面的路由器信息页面，如果 IPv6 那一段显示出来的地址里面有不是 `fe80` 这种的 IPv6 地址 (可能也会有 `fe80` 开头的，别全都是就行)，那就是成功开启了 IPv6。
    - TP-Link 路由器：登录后台后，点击底部路由设置，侧边栏找到 IPv6 设置点击，IPv6 功能调整为开启，WAN 口连接类型选择自动获取 IP 地址，点击保存。找到 IPv6 地址之后，跟上段华为路由器要求的效果一样就行。
    - 锐捷路由器：登录后台后，点击侧边栏的出口路由器，点击基本管理下的 IPv6 设置，打开。
      如果按照上面的方法设置之后路由器里也显示 `fe80` 的地址，证明你的光猫也需要配置，或者宽带不支持 IPv6。现在仍然有一些小运营商和一些地区不支持 IPv6，因为网络铺设的成本在那，IPv6 的普及还需要时间。
3. 检查路由器的上网方式：此时先打开路由器的后台，查看路由器的上网方式。不会操作可以百度路由器品牌 + 怎么设置拨号上网。
    - 华为路由器：点击顶部“我要上网”就可以看到上网方式。
    - TP-Link 路由器：点击底部路由设置，侧边栏找到上网设置，就可以看到上网方式。
    - 锐捷路由器：点击侧边栏的出口路由器，点击基本管理下的 Wan 配置，在联网方式处就可以看到上网方式
      如果这一步设置的是 DHCP（自动获取 IP 地址）或者动态 IP，那就需要修改光猫设置。如果这一步设置的已经是拨号上网或者宽带上网（PPPOE）且你的路由器已经打开了 IPv6，那就证明你的运营商没有给你开 IPv6 或者不支持 IPv6。你就可以直接跳过下面修改光猫的步骤，直接跟运营商报修说要开通 IPv6。
4. 获取光猫超级密码：如果有密码直接看下一步。如果安装师傅没给你超级密码，此时有以下三种解决办法：
    1. (成功率最高，但需要花钱) 在某宝某多某鱼搜索“光猫超级密码”，会有很多破解光猫超级密码的服务。这种方法会比较有效，但不是所有的光猫都能破解，在购买前应当咨询商家自己的光猫能否破解。特别对于超级密码是随机的，让商家禁用 TR069。这个过程可能会导致暂时的断网，请自行评估操作时是否具备断网的条件。
...
5. 关闭光猫 IPv6 防火墙并改桥接：进光猫后台后把 IPv6 防火墙关闭，再关闭 DHCP，最后改为桥接。具体修改方式因光猫不同会有所差异，还请自行上网搜索。
6. 使用路由器拨号：光猫改成桥接之后，进入路由器后台，将按上文查看上网方式的方法找到上网方式，改成拨号上网或宽带上网 (PPPoE)，填入在光猫后台找到的账号密码并保存。此时你家宽带的质量应该会有所提升，毕竟这次网关给到了路由器，自己的路由器性能一般都比运营商给的那个光猫的要好。
7. 折腾完路由器之后，回到服务器上，设备IP应该就会有公网 v6 地址了。要测试这个 v6 是不是真的生效了，可以访问 ipw.cn 或 testipv6.cn 测试。
